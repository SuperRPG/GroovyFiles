...
{
   std::vector<std::string> resouces{
      "file",
      "folder",
      "up",
      "edit-copy",
   ...
   config_t config{resouces};

   // Init SDL
   // if (! SDLUtils::init(&config))
   // {
   //    SDLUtils::close();
   //    return 1;
   // }

   // g_renderer = config.renderer;

   // Load icon textures
   // std::ostringstream oss;
   // oss << '/' << ICON_SIZE;
   // g_iconFile =      SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/file.png");
   // g_iconDir =       SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/folder.png");
   // g_iconUp =        SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/up.png");
   // g_iconCopy =      SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/edit-copy.png");
   // g_iconCut =       SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/edit-cut.png");
   // g_iconPaste =     SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/edit-paste.png");
   // g_iconTrash =     SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/trash.png");
   // g_iconDisk =      SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/disk.png");
   // g_iconSelect =    SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/select.png");
   // g_iconNone =      SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/none.png");
   // g_iconNewDir =    SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/folder-new.png");
   // g_iconQuit =      SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/quit.png");
   // g_iconEdit =      SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/edit.png");
   // g_iconCancel =    SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/cancel.png");
   // g_iconFloppy =    SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/floppy.png");
   // g_iconImage =     SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/image.png");
   // g_iconFileText =  SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/file-text.png");
   // g_iconPlus =      SDLUtils::loadTexture(&config, std::string(RES_PATH) + oss.str() + "/plus.png");

   // Load fonts
   config.font = SDLUtils::loadFont(std::string(RES_PATH) + "/" + FONT_NAME, FONT_SIZE);
   config.monoFont = SDLUtils::loadFont(std::string(RES_PATH) + "/" + FONT_NAME_MONO, FONT_SIZE);
   if (config.font == NULL || config.monoFont == NULL)
   {
      SDLUtils::close(&config);
      return 1;
   }

   // Width of one character in monospace font
   config.char_width = SDLUtils::getCharWidthMono(&config);

   // Execute main window
   {
      MainWindow mainWindow(&config, START_PATH);
      mainWindow.execute();
   }
